#!/bin/bash
# V2.23 Installationsscript für >>Bild verändern< als Nemo-Erweiterung vom 3.12.2021
# Dieses Script ist ab -Bild verändern- 2.23 erforderlich
...
#clear
echo "-----------------------------------------------------------------------"
echo "-Bild verändern- kann für verschieden Dateimanager installiert werden."
...
echo "Unter -Nemo- steht -Bild veändern- in zwei Betriebsarten zur Verfügung
...
Betriebsart -action-(Vorgabe) - Nemo                  (1)
Betriebsart -script- Nemo, Caja, Nautilus, Thunar     (2)
...
echo "$DateiManager" "action" "$breite" "$hoehe" "$InsStyle" > "$conf"
...
2)
 case "$DateiManager" in
   nemo)
    ...
    echo "$DateiManager" "script" "$breite" "$hoehe" "$InsStyle" > "$conf"
    echo "Die Scripte sind kopiert"
